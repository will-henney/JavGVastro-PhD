...
#IMAGEN
image_data=hdul[0].data
plt.imshow(image_data, cmap='RdBu_r')
ax.set_xlabel('X coordintate')
ax.set_ylabel('Y coordintate')
cbar = plt.colorbar()
...
sns.heatmap(
    ...
    vmin=-15, vmax=5,
    cmap="RdBu_r",cbar_kws={'label': 'km/s'})
ax.set_aspect("equal");
#plt.savefig('TAURUS/Imgs/A/'+reg+'RVH.png')
...
plt.text(0.91, 0.78,'$σ^{2}$ ='+str(np.round(data.RV.var(),2))+' km$^{2}$/s$^{2}$', ha='center', va='center', transform=ax.transAxes, color='k')
...
plt.savefig('Hist//Hubble X.pdf', bbox_inches='tight')
# -
...
datax = json.load(open("SFdata//HX.json"))
