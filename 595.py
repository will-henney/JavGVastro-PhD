...
# +
fig, ax = plt.subplots()
plt.scatter(np.log10(data.I),data.SigDisp, alpha=0.025, color='k',label='NGC 595')
...
fig, ax = plt.subplots()
plt.scatter(data.RV,data.SigDisp, alpha=0.015, color='k',label='NGC 595')
...
plt.text(0.70, 0.95,'n ='+str(data.RV.count()), ha='center', va='center', transform=ax.transAxes, color='k')
plt.text(0.70, 0.88,'$μ$ ='+str(np.round(data.RV.mean(),2))+' km/s', ha='center', va='center', transform=ax.transAxes, color='k')
plt.text(0.69, 0.80,'$σ^{2}$ ='+str(np.round(data.RV.var(),2))+' km$^{2}$/s$^{2}$', ha='center', va='center', transform=ax.transAxes, color='k')
...
plt.savefig('Hist//595.pdf', bbox_inches='tight')
# -
